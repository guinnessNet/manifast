...
Map flags these ("고아 문서 N개") and hides them by default. When you author or
...
5. **타입 분류(Diátaxis) + 대용량/다주제 청킹**: 각 문서를 카탈로그 타입으로 태깅한다. 한 페이지에
   타입이 섞였거나 **너무 크거나(여러 주제·버전·부록이 한 파일에 뭉쳐 있음)** 하면 주제별로 분할한다.
   앱은 큰 문서를 목록에서 첫 16KB만 읽으므로(뒤 내용은 프리뷰에 안 보임) 큰 다주제 문서일수록 분할 이득이 크다.
   - **원본 보존(raw)**: 통짜 원본은 `docs/archive/raw/`에 **verbatim 복사**로 보존한다(경로로 자동
     `archived` → 토글 뒤 숨김·신선도 점검 제외). 절대 삭제·덮어쓰지 않는다 — 출처/롤백 근거.
   - **본문 비편집**: 분할 조각의 본문은 원본을 **그대로 슬라이스**한다(재작성 금지 — "보류" 원칙). 조각마다
     frontmatter + **한 줄 출처 노트**(원본 raw 경로)만 덧붙인다.
   - **provenance 배선**: 각 조각에 `sources`(기술하는 코드 경로) + `related[]`로 **원본(raw)·형제 조각·인덱스**를
     잇는다(이러면 raw 포함 전부 linked → 고아 아님). 원본이 있던 경로는 짧은 **인덱스 문서**로 바꿔 부분들을
     가리켜 기존 참조/링크를 유지한다.
   - **적용 단계**: 새 파일 생성·본문 슬라이스는 "보류(제안만)"에 해당 — 사람 확인 후 작은 배치로 적용한다.
...
9. **관계 배선(고아 제거)**: 문서 간/문서↔스펙·태스크·화면 관계를 추적해 frontmatter로 엮는다
   (§6) — 구체 링크(spec→wireframe/tasks, task→`specId`)를 우선하고, 그걸로 표현 못 하는
   doc↔doc·doc→spec 관계는 `related[]`로 잇는다. **완료 기준: 문서의 90% 이상이 연결됨**
   (Map의 "고아 문서 N개" < 전체의 10%). ~90%에 도달하면 완료로 본다 — 남는 <10%는 진짜
   독립 문서(체인지로그·라이선스·일회성 메모)여야지, 단지 안 엮은 문서이면 안 된다. 숫자를
   맞추려 가짜 링크를 만들지 말 것. 실제로 무관한 문서는 고아로 남기는 게 옳다.
...
코드 변경이 끝날 때마다 `manifast-checklist` 스킬(`.claude/skills/manifast/CHECKLIST.md`)을 실행한다:
tasks.json 상태 업데이트 → plan.json 진행상황 → sources drift 확인 → 신규 기능 spec 생성 → 다이어그램 갱신.

기능 개발 전 과정(설계→계획→구현)을 자동화하려면 `manifast-workflow` 스킬(`.claude/skills/manifast/WORKFLOW.md`)을 사용한다.
